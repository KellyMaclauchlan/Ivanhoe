...
*** NOTE: We were unable to export a working runnable jar without having the source code next to it. Therefore the directories must stay in their original state for the client and server jars to run. We will attempt to remedy this for iteration 2. 
...
Once the server has started, type 'run' and press ENTER to start the server. 
...
Known issues:
- If the last tournament in a game is purple, the game winner is not announced
- Action cards are available and most have been implemented, however integration has not yet been done
